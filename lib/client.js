...
`}let O=[`slots`,`locale`];function k(e){e.effect(()=>{let e=document.createElement(`style`);return e.dataset.plugin=`@kenz1117/dsh-ui-rainbowspeak`,e.textContent=D(),document.head.appendChild(e),()=>{e.remove()}},`ui-rainbowspeak: rainbow border stylesheet`),e.effect(()=>(v(g()),()=>{let e=document.documentElement;delete e.dataset.rainbowOff,delete e.dataset.rainbowPalette,delete e.dataset.rainbowSide,delete e.dataset.rainbowBreathe,e.style.removeProperty(`--dsh-rainbow-period`),e.style.removeProperty(`--dsh-rainbow-width`),e.style.removeProperty(`--dsh-rainbow-direction`)}),`ui-rainbowspeak: initial preference application`),e.effect(()=>e.locale.register(w,{zh:T,en:E}),`ui-rainbowspeak: dictionaries`);let t=e.locale.bind(w);e.slots.inject(`settings.section`,()=>e.slots.register({name:`settings.section`,id:`rainbowspeak`,order:90,label:()=>t(`nav`),locale:w},C)),e.slots.inject(`conversation.composer.dock`,()=>e.slots.register({name:`conversation.composer.dock`,id:`rainbowspeak-state`,order:90},c))}return n.apply=k,n.inject=O,t.exports}});
